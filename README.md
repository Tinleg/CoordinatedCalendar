...
3. **Set up the pages in the sidebar.**
   - **Calendars:** in the middle card, choose the **Consolidated** calendar you just created, then set the **Busy block title** (default `Busy - Other`), whether gathered events keep their alerts (off by default), whether to skip events marked Free and meetings you declined (both on by default), and whether to skip all-day events (on by default). Hover over any of them for an explanation.
